...
class Manager extends DomainManager
{
    ...
    {
        return self::getTablePrefix() . 'campaigns';
    }
    ...
    public static function loadCollection(): Collection
    {
        ...
        $rows = self::getAdapter()->getArray(sprintf(
            'select * from %s order by key_campaign asc;',
            $name
        ));
    ...
    public static function load(string $key): Entity
    {
        ...
        $row = self::getAdapter()->getRow(sprintf(
            'select * from %s where key_campaign="%s"',
            $name,
            $key
        ));
    ...
    public static function save(Entity $entity): void
    {
        ...
        $key = $data['key_campaign'];
        unset($data['key_campaign']);

        self::getAdapter()->update(
            $name,
            $data,
            sprintf('key_campaign = "%s"', $key)
        );
    }
